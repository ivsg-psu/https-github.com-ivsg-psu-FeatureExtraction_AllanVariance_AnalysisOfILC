%%%%%%%%%%%%% script_classicalILC_InZeroDrift_OutWhiteNoise.m %%%%%%%%%%%%%
%% Purpose:
...
% true_plant_output --> Plant output w/o measurement noise
true_plant_output   = NaN(number_of_iterations,length_of_iteration,number_of_gamma);
% true_tracking_error --> Reference input - true_plant_output
true_tracking_error = NaN(number_of_iterations,length_of_iteration,number_of_gamma);
true_error_mse      = NaN(number_of_iterations,number_of_gamma);
...
for index_gamma = 1:number_of_gamma
    ...
    for index_iter = 1:number_of_iterations
        ...
        for index_time = 1:length_of_iteration
            ...
            % Output measurement
            plant_output(index_iter,index_time,index_gamma)      = C*system_state + white_noise(index_time); % Output at '(index_time-1)*sampling_interval'
            true_plant_output(index_iter,index_time,index_gamma) = C*system_state;
        ...
        true_tracking_error(index_iter,:,index_gamma) = reference_input-true_plant_output(index_iter,:,index_gamma);
        error_mse(index_iter,index_gamma)      = mean(tracking_error(index_iter,:,index_gamma).^2);
        true_error_mse(index_iter,index_gamma) = mean(true_tracking_error(index_iter,:,index_gamma).^2);
...
for index_gamma = 1:number_of_gamma
    ...
    hold on
    grid on
    plot(dis_time_vector,reference_input,'Color',[0.7, 0.7, 0.7],'LineWidth',5)
    legend_cell{1} = "Desired";
    for index_plot = 1:number_of_plots
        index_iter = iterations_to_plot(index_plot);
        plot(dis_time_vector,true_plant_output(index_iter,:,index_gamma),...
...
for index_gamma = 1:number_of_gamma
    ...
    for index_plot = 1:number_of_plots
        index_iter = iterations_to_plot(index_plot);
        plot(dis_time_vector,true_tracking_error(index_iter,:,index_gamma),...
            'Color',custom_color_map{index_plot},'LineWidth',1)
...
grid on
for index_gamma = 1:number_of_gamma
    plot(1:number_of_iterations,true_error_mse(:,index_gamma)',...
